/**
 * RevokeProgrammableConfigItemV1 tests for js-codama client
 *
 * Programmable Config Item delegates are metadata-level delegates that can manage
 * programmable configurations for specific items. They work for all non-edition token standards.
 *
...
/**
 * Test: Revoke programmable config item delegate for NonFungible
 */
test('it can revoke a programmable config item delegate for a NonFungible', async (t) => {
  ...
  // Approve a programmable config item delegate
  const programmableConfigItemDelegate = await createKeypair();
  const delegateInstruction = await getDelegateProgrammableConfigItemV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: programmableConfigItemDelegate.address,
  ...
  t.is(delegateRecord.data.delegate, programmableConfigItemDelegate.address);

  // Revoke the programmable config item delegate
  const revokeInstruction = await getRevokeProgrammableConfigItemV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: programmableConfigItemDelegate.address,
...
/**
 * Test: Revoke programmable config item delegate for ProgrammableNonFungible
 */
test('it can revoke a programmable config item delegate for a ProgrammableNonFungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke programmable config item delegate for Fungible
 */
test('it can revoke a programmable config item delegate for a Fungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke programmable config item delegate for FungibleAsset
 */
test('it can revoke a programmable config item delegate for a FungibleAsset', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
