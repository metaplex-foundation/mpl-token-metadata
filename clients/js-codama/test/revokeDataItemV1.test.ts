/**
 * RevokeDataItemV1 tests for js-codama client
 *
 * Data Item delegates are metadata-level delegates that can update data fields
 * for specific items. They work for all non-edition token standards.
 *
...
/**
 * Test: Revoke data item delegate for NonFungible
 */
test('it can revoke a data item delegate for a NonFungible', async (t) => {
  ...
  // Approve a data item delegate
  const dataItemDelegate = await createKeypair();
  const delegateInstruction = await getDelegateDataItemV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: dataItemDelegate.address,
  ...
  t.is(delegateRecord.data.delegate, dataItemDelegate.address);

  // Revoke the data item delegate
  const revokeInstruction = await getRevokeDataItemV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: dataItemDelegate.address,
...
/**
 * Test: Revoke data item delegate for ProgrammableNonFungible
 */
test('it can revoke a data item delegate for a ProgrammableNonFungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke data item delegate for Fungible
 */
test('it can revoke a data item delegate for a Fungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke data item delegate for FungibleAsset
 */
test('it can revoke a data item delegate for a FungibleAsset', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
