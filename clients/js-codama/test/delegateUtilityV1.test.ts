/**
 * DelegateUtilityV1 tests for js-codama client
 *
...
import { getDelegateUtilityV1InstructionAsync } from '../src/generated/instructions';
...
/**
 * Test: Approve utility delegate for ProgrammableNonFungible
 */
test('it can approve a utility delegate for a ProgrammableNonFungible', async (t) => {
  ...
  // Token record should have the delegate with Utility role
...
/**
 * Test: Cannot approve utility delegate for NonFungible
 */
test('it cannot approve a utility delegate for a NonFungible', async (t) => {
  ...
  // Try to approve a utility delegate (should fail)
  const utilityDelegate = await createKeypair();
  const delegateInstruction = await getDelegateUtilityV1InstructionAsync({
    mint: mint.address,
    tokenOwner: owner.address,
...
/**
 * Test: Cannot approve utility delegate for Fungible
 */
...
/**
 * Test: Cannot approve utility delegate for FungibleAsset
 */
