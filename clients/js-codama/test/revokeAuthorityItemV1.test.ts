/**
 * RevokeAuthorityItemV1 tests for js-codama client
 *
 * Authority Item delegates are metadata-level delegates that can manage
 * authority for specific items. They work for all non-edition token standards.
 *
...
/**
 * Test: Revoke authority item delegate for NonFungible
 */
test('it can revoke an authority item delegate for a NonFungible', async (t) => {
  ...
  const authorityItemDelegate = await createKeypair();
  const delegateInstruction = await getDelegateAuthorityItemV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: authorityItemDelegate.address,
  ...
  t.is(delegateRecord.data.delegate, authorityItemDelegate.address);

  // Revoke the authority item delegate
  const revokeInstruction = await getRevokeAuthorityItemV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: authorityItemDelegate.address,
...
/**
 * Test: Revoke authority item delegate for ProgrammableNonFungible
 */
test('it can revoke an authority item delegate for a ProgrammableNonFungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke authority item delegate for Fungible
 */
test('it can revoke an authority item delegate for a Fungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke authority item delegate for FungibleAsset
 */
test('it can revoke an authority item delegate for a FungibleAsset', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
