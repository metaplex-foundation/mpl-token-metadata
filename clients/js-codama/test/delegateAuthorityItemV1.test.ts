/**
 * DelegateAuthorityItemV1 tests for js-codama client
 *
 * AuthorityItem delegates are metadata-level delegates.
...
import { getDelegateAuthorityItemV1InstructionAsync } from '../src/generated/instructions';
...
/**
 * Test: Approve authority item delegate for NonFungible
 */
test('it can approve an authority item delegate for a NonFungible', async (t) => {
...
/**
 * Test: Approve authority item delegate for ProgrammableNonFungible
 */
test('it can approve an authority item delegate for a ProgrammableNonFungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Approve authority item delegate for Fungible
 */
test('it can approve an authority item delegate for a Fungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Approve authority item delegate for FungibleAsset
 */
test('it can approve an authority item delegate for a FungibleAsset', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
