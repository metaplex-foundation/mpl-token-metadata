/**
 * RevokeStakingV1 tests for js-codama client
 *
...
/**
 * Test: Revoke staking delegate for ProgrammableNonFungible
 */
test('it can revoke a staking delegate for a ProgrammableNonFungible', async (t) => {
  ...
  // Revoke the staking delegate
  const revokeInstruction = await getRevokeStakingV1InstructionAsync({
    mint: mint.address,
    tokenOwner: owner.address,
    authority: owner,
    payer: owner,
...
/**
 * Test: Cannot revoke staking delegate for NonFungible
 */
test('it cannot revoke a staking delegate for a NonFungible', async (t) => {
  ...
  // Try to revoke using staking revoke (should fail - wrong delegate type)
  const stakingDelegate = await createKeypair();
  const revokeInstruction = await getRevokeStakingV1InstructionAsync({
    mint: mint.address,
    tokenOwner: owner.address,
...
/**
 * Test: Cannot revoke staking delegate for Fungible
 */
...
/**
 * Test: Cannot revoke staking delegate for FungibleAsset
 */
