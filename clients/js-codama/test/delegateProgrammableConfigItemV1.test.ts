/**
 * DelegateProgrammableConfigItemV1 tests for js-codama client
 *
 * ProgrammableConfigItem delegates are metadata-level delegates.
...
import { getDelegateProgrammableConfigItemV1InstructionAsync } from '../src/generated/instructions';
...
/**
 * Test: Approve programmable config item delegate for NonFungible
 */
test('it can approve a programmable config item delegate for a NonFungible', async (t) => {
...
/**
 * Test: Approve programmable config item delegate for ProgrammableNonFungible
 */
test('it can approve a programmable config item delegate for a ProgrammableNonFungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Approve programmable config item delegate for Fungible
 */
test('it can approve a programmable config item delegate for a Fungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Approve programmable config item delegate for FungibleAsset
 */
test('it can approve a programmable config item delegate for a FungibleAsset', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
