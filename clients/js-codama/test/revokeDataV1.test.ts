/**
 * RevokeDataV1 tests for js-codama client
 *
...
/**
 * Test: Revoke data delegate for NonFungible
 */
test('it can revoke a data delegate for a NonFungible', async (t) => {
  ...
  // Revoke the data delegate
  const revokeInstruction = await getRevokeDataV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: dataDelegate.address,
...
/**
 * Test: Revoke data delegate for ProgrammableNonFungible
 */
test('it can revoke a data delegate for a ProgrammableNonFungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke data delegate for Fungible
 */
test('it can revoke a data delegate for a Fungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke data delegate for FungibleAsset
 */
test('it can revoke a data delegate for a FungibleAsset', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
