/**
 * RevokeCollectionItemV1 tests for js-codama client
 *
 * Collection Item delegates are metadata-level delegates that can manage
 * collections for specific items. They work for all non-edition token standards.
 *
...
/**
 * Test: Revoke collection item delegate for NonFungible
 */
test('it can revoke a collection item delegate for a NonFungible', async (t) => {
  ...
  // Approve a collection item delegate
  const collectionItemDelegate = await createKeypair();
  const delegateInstruction = await getDelegateCollectionItemV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: collectionItemDelegate.address,
  ...
  t.is(delegateRecord.data.delegate, collectionItemDelegate.address);

  // Revoke the collection item delegate
  const revokeInstruction = await getRevokeCollectionItemV1InstructionAsync({
    mint: mint.address,
    authority: updateAuthority,
    payer: updateAuthority,
    delegate: collectionItemDelegate.address,
...
/**
 * Test: Revoke collection item delegate for ProgrammableNonFungible
 */
test('it can revoke a collection item delegate for a ProgrammableNonFungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke collection item delegate for Fungible
 */
test('it can revoke a collection item delegate for a Fungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Revoke collection item delegate for FungibleAsset
 */
test('it can revoke a collection item delegate for a FungibleAsset', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
