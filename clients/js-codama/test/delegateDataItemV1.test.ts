/**
 * DelegateDataItemV1 tests for js-codama client
 *
 * DataItem delegates are metadata-level delegates.
...
import { getDelegateDataItemV1InstructionAsync } from '../src/generated/instructions';
...
/**
 * Test: Approve data item delegate for NonFungible
 */
test('it can approve a data item delegate for a NonFungible', async (t) => {
...
/**
 * Test: Approve data item delegate for ProgrammableNonFungible
 */
test('it can approve a data item delegate for a ProgrammableNonFungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Approve data item delegate for Fungible
 */
test('it can approve a data item delegate for a Fungible', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
...
/**
 * Test: Approve data item delegate for FungibleAsset
 */
test('it can approve a data item delegate for a FungibleAsset', async (t) => {
  const canRun = await canRunTests();
  if (!canRun) {
    t.log(getSkipMessage());
    t.pass('Skipped - validator not running');
