/**
 * RevokeTransferV1 tests for js-codama client
 *
...
/**
 * Test: Revoke transfer delegate for ProgrammableNonFungible
 */
test('it can revoke a transfer delegate for a ProgrammableNonFungible', async (t) => {
  ...
  // Revoke the transfer delegate
  const revokeInstruction = await getRevokeTransferV1InstructionAsync({
    mint: mint.address,
    tokenOwner: owner.address,
    authority: owner,
    payer: owner,
...
/**
 * Test: Cannot revoke transfer delegate for NonFungible
 */
test('it cannot revoke a transfer delegate for a NonFungible', async (t) => {
  ...
  // Try to revoke using transfer revoke (should fail - wrong delegate type)
  const transferDelegate = await createKeypair();
  const revokeInstruction = await getRevokeTransferV1InstructionAsync({
    mint: mint.address,
    tokenOwner: owner.address,
...
/**
 * Test: Cannot revoke transfer delegate for Fungible
 */
...
/**
 * Test: Cannot revoke transfer delegate for FungibleAsset
 */
