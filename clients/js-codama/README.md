...
This is the Codama-generated TypeScript client for mpl-token-metadata, using the new `@solana/kit` API (Solana web3.js 2.0).
...
The `clients/js` directory uses the **Umi framework** (older API), while this `clients/js-codama` uses **@solana/kit** (web3.js 2.0). Key differences:

| Aspect | clients/js (Umi) | clients/js-codama (@solana/kit) |
|--------|------------------|----------------------------------|
...
# From clients/js-codama, run tests
pnpm test
```
...
This will run `configs/codama.cjs` which generates code in `clients/js-codama/src/generated/`.
