...
import { createV1, findMetadataPda, mintV1, mplTokenMetadata, TokenStandard, verifyCreatorV1 } from '../src';
...
export const createDigitalAssetWithVerifiedCreators = async (
  ...
  const mint = generateSigner(umi);
  await transactionBuilder().add(createV1(umi, {
    mint,
    name: 'My NFT',
    uri: 'https://example.com',
    sellerFeeBasisPoints: percentAmount(2.5),
  ...
  await transactionBuilder().add(verifyCreatorV1(umi, {metadata, authority: input.creatorAuthority})).add(mintV1(umi, {
    authority: input.authority,
    mint: mint.publicKey,
    amount: 1,
