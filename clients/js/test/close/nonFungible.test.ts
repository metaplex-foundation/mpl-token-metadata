...
import { addAmounts, createSignerFromKeypair, generateSigner, lamports, MaybeRpcAccount, publicKey, subtractAmounts } from '@metaplex-foundation/umi';
import { readFileSync } from 'fs';
import { burnToken } from '@metaplex-foundation/mpl-toolbox';
import { createDigitalAssetWithToken, createUmi } from '../_setup';
import { closeAccounts, fetchDigitalAssetWithAssociatedToken, printSupply, printV1, TokenStandard } from '../../src';
...
test.skip('it can close ownerless metadata for a non-fungible with zero supply', async (t) => {
    ...
    const closeAuthority = createSignerFromKeypair(umi, umi.eddsa.createKeypairFromSecretKey(
        new Uint8Array(
            JSON.parse(
                readFileSync(
    ...
    const asset = await fetchDigitalAssetWithAssociatedToken(umi, mint.publicKey, umi.identity.publicKey);
    ...
    t.deepEqual(await umi.rpc.getAccount(asset.metadata.publicKey), <MaybeRpcAccount>{
...
test.skip('it can close ownerless metadata for a non-fungible edition with zero supply', async (t) => {
    ...
    const asset = await fetchDigitalAssetWithAssociatedToken(umi, editionMint.publicKey, umi.identity.publicKey);
