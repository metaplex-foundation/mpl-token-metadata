...
> If a bump seed is known, it is _cheaper_ (in terms of compute units) to use the `create_pda` function, in particular for on-chain code.
 ...
 > **Note:**
 > Although the SDK has 5 dependencies, in practice the only "real" dependency is the `solana-program` crate since the remaining 4 dependencies are also dependencies from the `solana-program`.
