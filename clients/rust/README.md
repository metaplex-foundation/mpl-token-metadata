...
## Why using a client library (SDK)?
 ...
 > **Note**
 > Although the SDK crate has 3 dependencies, in practice the only "real" dependency is the `solana-program` crate since the remaining dependencies are also dependencies of the `solana-program`.
